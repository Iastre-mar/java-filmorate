...
CREATE TABLE IF NOT EXISTS ref_director
(
    id   BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name VARCHAR(50) NOT NULL UNIQUE
);
...
(
    ...
    FOREIGN KEY (film_id) REFERENCES films (id) ON DELETE CASCADE,
    FOREIGN KEY (director_id) REFERENCES ref_director (id) ON DELETE CASCADE
);
...
-- Создание индексов
CREATE INDEX IF NOT EXISTS uidx_film_genre ON film_genres (film_id, genre_id);
CREATE INDEX IF NOT EXISTS uidx_film_user_like ON film_likes (film_id, user_id);
